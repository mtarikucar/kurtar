...
async function bootstrap() {
  ...
  // [Task 9.5] See config/cors-origins.ts — additive, origin-allowlisted,
  // credentials-enabled CORS for the four frontend dev origins (or
  // CORS_ALLOWED_ORIGINS's explicit override). Skipped entirely (no
  // enableCors() call at all) when neither applies, which preserves the
  // exact pre-Task-9.5 behavior. It lives in its own module because this
  // file calls bootstrap() at module scope and so cannot be imported by a
  // spec.
