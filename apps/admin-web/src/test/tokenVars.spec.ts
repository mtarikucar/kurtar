...
/**
 * [M8 fix / M21 cross-cutting guard] Every `.module.css` file in this app
 * is meant to read colors/spacing/radii/type ONLY through the CSS custom
 * properties `styles/tokens.ts`'s `injectDesignTokens()` actually sets on
 * `:root` — that function is the single bridge from `@kurtar/ui-tokens`
 * into real CSS. A `var(--color-danger-900)` reference that the injector
 * never produces isn't a type error and isn't a lint error: the shorthand
 * property it sits in just silently falls back to its CSS-spec initial
 * value at computed-value time (e.g. `border: 2px solid var(--undefined)`
 * -> `border-style: none`) — exactly what happened to the breached SLA
 * badge (M8) before its fix. This test re-derives the exact same variable
 * names `injectDesignTokens()` would set and fails loudly the moment any
 * `.module.css` file references one that isn't in that set, instead of
 * relying on a human to notice a badge quietly lost its border.
 */
...
describe("every var(--...) in a *.module.css file is a token injectDesignTokens() actually sets", () => {
