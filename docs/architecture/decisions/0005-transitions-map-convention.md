...
- **The convention is opt-in per file** — a future entity that skips it (hand-typing its own WHERE clauses again) reintroduces exactly the drift risk this exists to close. Every stateful entity added since Task 5 (complaints, content reports, settlement batches) has followed it; keep doing so.
