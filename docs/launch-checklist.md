...
## Session & actor boundaries

- [x] **[ENGINEER]** *(closed — recorded here because it was found late and is the class of thing a future reader must be able to find)* **Cross-actor session bleed.** The refresh token used to travel in ONE cookie — `refreshToken`, path `/api/auth`, with no actor anywhere in it — while all four surfaces share a single backend origin. A cookie is scoped by host+path, never by port and never by which frontend set it, so that meant: whichever actor signed in LAST owned the browser's only session; logging out of one surface killed the other's; admin-web's mount-time session restore rendered an authenticated admin shell off a *merchant's* cookie; and `httpOnly` bought nothing across the boundary, because any script on any same-site surface could `fetch("/api/auth/refresh", {credentials:"include"})` and be handed an access token for whoever had most recently authenticated. Fixed at the backend cookie layer, not in the apps' session-restore: per-actor cookie name **and** path (`refreshToken_admin` at `/api/auth/admin`, and so on), one refresh/logout route per actor, and a server-side check that the presented token's own `principalType` matches the route it arrived on. Proven by `backend/src/modules/auth/auth-refresh-actor-binding.realdb.spec.ts` — a real merchant refresh token cannot mint an ADMIN session, and rejecting the attempt does not revoke the merchant's own family.
- [ ] **[ENGINEER]** At go-live, confirm the real origin topology: every surface served from an origin listed in `CORS_ALLOWED_ORIGINS`, and the refresh cookies actually `Secure` (they are, via `NODE_ENV=production`). The actor scoping above is defence in depth, not a substitute for getting the origin/CORS topology right.
- [ ] **[ENGINEER]** Browsers that authenticated before the fix still hold a stale, unscoped `refreshToken` cookie. Every auth response now emits a `Set-Cookie` that expires it, so it self-clears on the next login/refresh/logout — confirm once on the real production domain (DevTools → Application → Cookies) rather than assuming.

## Merchant lifecycle — kill-switch has no way back

- [ ] **[ENGINEER]** **Build a reinstate path for a `SUSPENDED` merchant, or accept in writing that suspension is permanent.** `merchant-verification-transitions.ts` makes `SUSPENDED` a genuinely terminal state (`SUSPENDED: []` — no transition out, by design, per its own comment) and `POST /api/admin/merchants/:id/approve` only accepts `SUBMITTED`/`UNDER_REVIEW`, so calling it on a suspended merchant 409s instead of reinstating them. `docs/operations.md`'s kill-switch section used to claim re-approval "restores write access" — it does not, and has been corrected. As shipped, an admin clicking suspend ends that merchant's ability to transact on the platform forever, with the only way back being a manual, unaudited direct database write. That is a real product gap for a control this easy to click by mistake or over-apply to something that should have been temporary (a documentation dispute, a first complaint) — either ship a real reinstate endpoint (its own transition, its own audit event, matching the pattern the other three transitions already follow) before go-live, or have the founder explicitly sign off that "suspend = permanent, no undo" is the intended launch behavior.
...
- [ ] **[ENGINEER]** Set the `PUBLIC_API_BASE_URL` GitHub Actions repository VARIABLE (not a secret) to the real production backend origin before the first tag-triggered release. `.github/workflows/release-deploy.yml` now builds and pushes all three frontend images (merchant-web/admin-web/landing — previously only the backend image was built, so `docker compose pull` on the prod host 404'd on the other three the moment `ops/docker-compose.prod.yml` started referencing them) and passes this variable as each one's `VITE_API_BASE_URL`/`NEXT_PUBLIC_API_BASE_URL` build-arg, since both Vite and Next inline that value into the client bundle at BUILD time. Left unset, every built frontend ships pointed at `http://localhost:4750` — wrong for any real user. This whole workflow is still INERT (no GitHub remote configured yet) and unexercised by a real run; the automated rollback also still only reverts the `api` container, not the three frontend ones (documented in the workflow's own top comment) — treat as reviewed-not-proven until one real tag-triggered deploy succeeds.
...
- [ ] **[ENGINEER]** The consumer app's offline redeem fallback (`apps/consumer/src/lib/redeem-queue.ts`) is a dead end once queued: `POST /reservations/:id/redeem` is only ever called from the swipe itself, and once a swipe is queued the redeem screen never renders the swipe control again — reconciliation is a pure read-only poll of `GET /reservations/mine`. A consumer stuck offline at the counter has no self-serve way to complete their own redemption; it only completes if staff redeem it manually from the merchant-web pickup list. Real fix is either an automatic retry of the direct call on reconnect, or a visible "retry now" affordance on the redeem screen — not implemented, both doc comments corrected to stop claiming an automatic retry exists.
