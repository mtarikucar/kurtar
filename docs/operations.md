...
4. A batch `SENT` for more than 3 days without reaching `SETTLED` surfaces on the 09:00 reconciliation sweep — treat that as "confirm with the PSP/bank directly," not "retry again." Note that nothing in the product writes `SETTLED` today (there is no bank/PSP reconciliation feed), so this branch alerts **once per batch** and then stays quiet; it is a prompt to reconcile manually, not a state the system will ever clear on its own.
...
6. Every money-moving admin action here — approve, hold, retry, run-nightly, and platform pricing changes — writes an `AuditLog` row (`actorType: "ADMIN"`, the admin's id, `settlement.approved` / `settlement.held` / `settlement.retried` / `settlement.nightly_run` / `pricing.scheduled`). "Who approved this payout" is a plain `audit_logs` query by `entityId`.
