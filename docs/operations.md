...
All twelve crons run inside the single `api` container (NestJS's `@nestjs/schedule`, in-process — see [`docs/architecture/decisions/0002-outbox-pattern.md`](architecture/decisions/0002-outbox-pattern.md) for why this is fine at kurtar's current scale, and what to revisit if the api service is ever scaled past one replica). Only **two of the twelve** — both settlement-related — have an admin-triggerable on-demand equivalent today (noted per row below); the other ten have no way to force a run without waiting for the schedule, which is worth knowing before assuming you can nudge one along during an incident.
...
| **No-show sweep** (`no-show-sweeper.service.ts`) | every 10 min | Moves a `CONFIRMED` reservation to the terminal `NO_SHOW` once its pickup window closed more than 1h ago, in bounded batches of 500, through the transitions table's guarded update. Nobody is refunded and nothing else is written: a no-show is a normal sale (plan §4.3), so the bag then settles to the merchant on the same terms as a redeemed one. | **Merchants stop being paid for bags nobody collected.** The reservation sits at `CONFIRMED` for ever, the settlement scan never sees it (it only takes `REDEEMED`/`NO_SHOW`), and the consumer's app keeps the order in AKTİF under a dead countdown. The 1h grace is deliberately short: a no-show swept after its own day's 02:00 batch has been approved/sent can no longer be attached to it. |
...
| **Settlement nightly batch** (`settlement-batch-builder.service.ts`) | 02:00 Europe/Istanbul | Groups the day's `REDEEMED`-or-`NO_SHOW`-with-no-settlement-line reservations per merchant per day (a no-show is grouped by the Istanbul day its pickup window closed on — the same day as its redeemed siblings), computes each batch (`CALCULATED` or `HELD` if fees exceed gross). On-demand: `POST /api/admin/settlements/run-nightly`. | Redeemed and uncollected bags alike never get batched for payout — merchants don't get paid. This is the entry point to the whole settlement pipeline; see "reading a settlement batch" below. |
